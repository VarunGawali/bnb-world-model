...
nodes. For cut selection, a pointer network scores candidate Gomory cuts against the
same latent state used for branching, so both decisions share a single
encoder rather than requiring separate inference passes. A single unified model
