...
the imagined child state for a k-step latent lookahead. Candidates are scored
by combining the predicted child node's dual-bound value with a learned
subtree-size penalty that directly targets the solver's true cost in explored nodes. For cut selection, candidate Gomory cuts are scored via cross-attention against
the encoder's existing variable and constraint representations, so branching
and cut selection share a single latent without any additional learned
parameters or separate inference passes. A single unified model
...
Depth-1 rollout explores $44\%$ fewer nodes than imitation alone ($3{,}197$
...
configuration when wall time is the constraint. Depth-3 rollout regresses
($+14\%$ nodes over depth-1), reflecting accumulated dynamics error; depth~1
is the correct horizon at current model fidelity.
...
\emph{solution quality}: combining cuts with depth-1 rollout (Neural-Best)
reduces the primal gap from $5.2\%$ (Rollout-d1 alone) to $1.9\%$ at medium
...
Depth-1 rollout cuts SGM nodes by $44\%$ vs.\ imitation alone ($3{,}197$
...
\textbf{Depth and size head have scale-dependent limits.}
Depth-3 rollout regresses at medium ($+14\%$ nodes) but helps at hard ($-4.9\%$
vs.\ depth-1), suggesting dynamics fidelity, not rollout depth, is the
binding constraint. The subtree-size head reverses its easy-tier gain at medium
...
Yes. The abstract and introduction claim (a) a latent dynamics model for B\&C that replaces child LP solves with a cheap forward pass, (b) a unified model driving branching, cut selection, and node ordering with an exactness guarantee, (c) 44\% node reduction via depth-1 latent rollout on medium-tier Set Cover, and (d) 100\% tree-exhaustion rate on instances that time out all classical rules. All claims are supported directly by Tables~1--2 and the ablation in Section~5.3. ``Terminated'' in the tables means the search tree was exhausted within the budget; this is not a proof of optimality, and the paper's gap column is computed relative to the best solution found by any method within the same budget (footnote $\ddagger$), not against a verified optimum.
