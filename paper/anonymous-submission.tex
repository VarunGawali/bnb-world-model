...
10~instances, $300$\,s limit). Five findings summarize the results.

\textbf{(1) Solve-rate discontinuity.} All four classical baselines time out on
every instance (0/10 solved), while every neural variant proves optimality on
all 10 instances within the $300$\,s budget. SCIP, given three times the budget
($900$\,s), still closes zero instances and leaves a mean duality gap of
$16.7\%$. The neural solver is not marginally more efficient; it certifies
integer optima on problem sizes where all tested classical methods cannot
terminate.

\textbf{(2) Neural-Best vs.\ SCIP.} The full model (Neural-Best: rollout~+
attention cuts) proves optimality in SGM $242$\,s --- comfortably within the
$300$\,s budget --- while SCIP needs more than $900$\,s and still fails.
This $3.7\times$ wall-time advantage comes with provably optimal certificates,
not an approximation gap.

\textbf{(3) Rollout overhead is justified.} The depth-1 latent rollout
(Rollout-d1) halves explored nodes relative to the imitation policy: SGM
$3{,}197$ vs.\ $5{,}733$, a $\mathbf{44\%}$ reduction. The full model
(Neural-Best), which adds attention-guided Gomory cuts, preserves this gain
($3{,}235$ nodes, $242$\,s), confirming that Gomory cuts tighten the LP bound
at the root and prune the tree without competing with the rollout's branching
efficiency. Neural-Best thus achieves the best combined performance: rollout's
node efficiency plus root cuts, in $242$\,s.

\textbf{(4) MF-Blend is the fastest.} Blending policy logits with
most-fractional scores (MF-Blend) is the fastest neural configuration: $218$\,s
with SGM $4{,}165$ nodes. It diversifies the candidate ranking at near-zero
overhead, capturing a substantial fraction of the rollout's node reduction
without the latent inference cost, and is attractive when wall time is
the primary constraint.

\textbf{(5) Deeper rollout hurts.} Increasing rollout depth from 1 to 3
(Neural-Best-d3) \emph{increases} SGM nodes to $3{,}697$ ($+14\%$ over d1)
and adds $15$\,s per instance. The dynamics model is trained for single-step
fidelity, and compounding its predictions over three steps accumulates enough
error to misrank candidates. Depth~1 is thus not a computational shortcut
but the \emph{correct} horizon given the model's current accuracy; improving
multi-step dynamics fidelity is the direct path to deeper planning.
...
The results above map cleanly onto five takeaways.

\textbf{Solve-rate gap, not a node-count gap.} The dominant finding is not
that neural solvers explore fewer nodes than classical ones; it is that they
\emph{terminate} while classical solvers do not. All four classical baselines
time out on every medium-tier instance, and SCIP --- given three times the
budget --- still leaves a $16.7\%$ duality gap. The neural overhead is repaid
not by being faster per node but by needing far fewer nodes to close the proof.

\textbf{Our solver proves optimality in 242s where SCIP needs $>$900s.}
Neural-Best solves every instance in SGM $242$\,s, a $3.7\times$ wall-time
advantage over SCIP's unsuccessful $900$\,s run. This is the headline
practical result: CutWorld delivers certified optima on instances SCIP cannot
close under any reasonable budget.

\textbf{Rollout overhead is justified: it halves nodes.} The latent rollout
(Rollout-d1) reduces explored nodes by $44\%$ over the imitation policy
($3{,}197$ vs.\ $5{,}733$), repaying its $\sim\!34$\,s additional inference
cost in a comfortably smaller search tree. Neural-Best preserves this gain
($3{,}235$ nodes) while additionally applying attention-guided Gomory cuts that
tighten the LP bound at the root; the combination confirms that root cuts and
rollout-guided branching are complementary, not competing.

\textbf{MF-Blend is the fastest option.} Blending policy logits with
most-fractional scores runs in SGM $218$\,s with $4{,}165$ nodes, making it
the best choice when wall time is the primary constraint. It shows that a cheap
signal-diversity trick captures a large share of the rollout's node reduction
at a fraction of the cost, and is a natural baseline for any latent-rollout
ablation.

\textbf{Deeper rollout is actively harmful.} Depth-3 rollout increases nodes
by $+14\%$ over depth-1 (SGM $3{,}697$ vs.\ $3{,}197$). The dynamics model is
trained for one-step accuracy; compounding its predictions over three steps
accumulates enough error to misrank candidates. The correct conclusion is not
that planning depth should be capped at 1 indefinitely, but that improving
multi-step dynamics fidelity --- through more aggressive latent overshooting,
contrastive state grounding, or a larger Transformer --- is the direct lever
for unlocking deeper planning. Learned cut selection at a tight budget
(Table~\ref{tab:cuts}) provides complementary evidence that latent evaluation
is most reliable when the candidate set is small and the imagination horizon is
short. Together, these results map the current boundary of what a learned
latent dynamics model can and cannot yet do inside an exact solver, while
preserving optimality throughout.
