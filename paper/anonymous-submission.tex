...
nodes. For cut selection, candidate Gomory cuts are scored via cross-attention against
...
all are optimized in the four-phase curriculum of Section~\ref{sec:training}.

\begin{table}[h]
\centering
\caption{Per-head training objectives.  BCE$^+$ = weighted binary cross-entropy with positive-class weight equal to the negative-to-positive ratio.  $\dagger$~Joint fine-tuning phase only.}
\label{tab:losses}
...
Every neural variant terminates on $100\%$ of instances; all four classical
rules exhaust the budget. The classical node counts (marked $\dagger$) are
...
multi-step accuracy should unlock further gains. The gap-column evidence in Tables~\ref{tab:main} and~\ref{tab:hard} provides
parallel support: latent evaluation is most reliable over a short horizon when
the candidate set is small, but the benefit of depth grows as the branching
factor and tree depth increase.
...
mixed precision on GPU; four-stage curriculum with per-stage learning rate,
