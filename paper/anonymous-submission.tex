...
Within a $300$\,s budget, every neural variant terminates on all 10 instances
while every classical rule exhausts the budget. The neural efficiency advantage
is $10$--$25\%$ in wall time, which is real but not dramatic; what makes it
consequential is that it falls on the correct side of the budget cutoff. The
classical node counts in Table~\ref{tab:main} (left column, measured at timeout)
are lower bounds on what a completed solve would explore; SB-8's $2{,}710$ looks
competitive with neural counts in the same column only because it is an
incomplete count. Putting the two blocks in separate columns is therefore not a
formatting choice but a semantic one.

\textbf{Depth-1 rollout is the single most powerful component.} The $44\%$
SGM-node reduction from Rollout-d1 ($3{,}197$ vs.\ $5{,}733$) is the largest
...
bound at the root; the combination confirms that rollout and cuts are
complementary. Cuts alone, without rollout, yield only $-6\%$ ($5{,}396$
nodes), consistent with root cuts having limited leverage when the LP relaxation
is already relatively tight.

\textbf{The subtree-size head does not generalise to medium scale.}
Rollout-d1+Size reverses its easy-tier gain, increasing nodes from $3{,}197$ to
$4{,}320$ ($+35\%$). The head was trained on trajectories from small trees and
is extrapolating beyond its distribution at medium scale; this is the sharpest
evidence that the training distribution is a binding constraint for the planning
components, and expanding it is the most direct path to generalisation.

\textbf{Deeper rollout is actively harmful at current dynamics fidelity.}
Depth-3 rollout ($3{,}697$ nodes, $+14\%$ over depth-1) confirms that the
dynamics model's single-step fidelity does not compose cleanly over three steps.
This is not a reason to avoid planning depth; it is a precise specification of
what must improve --- multi-step dynamics accuracy --- before deeper planning
helps. Learned cut selection at a tight budget (Table~\ref{tab:cuts}) provides
parallel evidence: latent evaluation is most reliable on a small candidate set
over a short horizon. Together, these results map where a latent dynamics model
currently helps inside an exact solver and where it does not, while the
exactness guarantee holds throughout.
