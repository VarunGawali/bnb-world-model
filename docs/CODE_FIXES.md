...
**Follow-up (open):** the *inference* rollout (`world_model.rollout_candidate` → `step_full`) still passes direction 0, so expanding a candidate does not yet explore up vs down as distinct children. Training is now direction-conditioned; wiring real directions (or expanding both children) into the inference rollout is the remaining step for full fidelity. Note: widening `input_proj` changes the dynamics checkpoint shape — Phase-3 weights must be retrained (encoder/policy checkpoints unaffected).
