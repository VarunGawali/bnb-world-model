...
| P0.3 wrong transitions | **REAL** | Collector records Ecole `Branching` visitation order; npz has `depths` but no `node_id/parent_id`. `datasets.py:370` uses `z[1:]` as "next". | Record `node_id`, `parent_id`, `branch_dir` per node. In the dataset, build dynamics pairs only where `child.parent_id == node.node_id`. Each child's own LP state is already recorded when SCIP visits it — no double-probing needed. |
