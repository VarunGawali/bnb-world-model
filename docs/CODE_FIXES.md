...
| P0.3 wrong transitions | **REAL** | Collector records Ecole `Branching` visitation order; npz has `depths` but no `node_id/parent_id`. `datasets.py:370` uses `z[1:]` as "next". | Collector now records `node_ids`/`parent_ids` (done, v2). **Dataset fix is NOT a simple reorder:** the dynamics model is a *causal Transformer* (`dynamics.py`), so feeding arbitrary parent->child edges as a sequence pollutes each edge's causal context with unrelated edges. Correct options: **(A, recommended)** reconstruct each node's true root->node path via `parent_ids` and train the Transformer on that ordered path (keeps the multi-step rollout claim; cap ancestor window); **(B)** single-step dynamics with history disabled at train+inference (also resolves P0.9, but drops the sequence narrative). **Decision pending; `datasets.py` left unchanged until chosen and real data exists.** |
