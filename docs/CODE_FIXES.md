...
| P0.11 leaf/bound targets | **FIXED** | Leaf labels: real (node with no recorded children) in v2 collector. Bound target: `gap_to_primal_norm` computes `(db-root)/(primal-root)` from the stored anchors at dataset-load — a fixed cross-instance scale (0 at root, 1 at optimum), replacing per-trajectory min-max. Falls back to `norm_dual_bounds` for legacy/degenerate files. Used by both TransitionDataset and SequenceDataset. |
